...
So if it crashes it may be due to the lack of power.
...
Next I leave the videos of the circuits. **Note to read the disclaimer** because this results may vary a lot deppending on the machine or the load of it. It may cause some crashes in Montmelo and Montreal chicanes.
...
while True:
    ...
    if len(contours) > 0:
      ...
      HAL.setW(pid.getOutput(320 - center_x))
      lin_speed = pid2.getOutput(320 - abs(320 - center_x))
      HAL.setV(lin_speed)
    GUI.showImage(np.hstack([image, output]))
```
